...
public class AccountHistoryParser implements HuobiModelParser<AccountHistory> {
    ...
    public AccountHistory parse(JSONObject json) {
        return AccountHistory.builder().accountId(json.getLong("account-id")).currency(json.getString("currency"))
                .transactAmt(json.getBigDecimal("transact-amt")).transactType(json.getString("transact-type"))
                .availBalance(json.getBigDecimal("avail-balance")).acctBalance(json.getBigDecimal("acct-balance"))
                .transactTime(json.getLong("transact-time")).recordId(json.getLong("record-id")).build();
    }
