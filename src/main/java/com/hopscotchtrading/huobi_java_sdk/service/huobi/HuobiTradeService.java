...
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.BatchCancelOpenOrdersResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.BatchCancelOrderResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.FeeRateParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.MatchResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.OrderParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.OrderUpdateEventV2Parser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.trade.TradeClearingEventParser;
...
public class HuobiTradeService implements TradeClient {
    ...
    public Long createOrder(CreateOrderRequest request) {
        ...
        InputChecker.checker().checkSymbol(request.getSymbol()).shouldNotNull(request.getAccountId(), "Account-Id")
                .shouldNotNull(request.getAmount(), "Amount").shouldNotNull(request.getType(), "Type")
                .shouldNotNull(request.getOrderSource(), "order source");

        if (request.getType() == OrderTypeEnum.SELL_LIMIT || request.getType() == OrderTypeEnum.BUY_LIMIT
                || request.getType() == OrderTypeEnum.BUY_LIMIT_MAKER
                || request.getType() == OrderTypeEnum.SELL_LIMIT_MAKER) {
            InputChecker.checker().shouldNotNull(request.getPrice(), "Price");
        }
        if (request.getType() == OrderTypeEnum.SELL_MARKET || request.getType() == OrderTypeEnum.BUY_MARKET) {
            InputChecker.checker().shouldNull(request.getPrice(), "Price");
        }
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("account-id", request.getAccountId())
                .putToPost("amount", request.getAmount()).putToPost("price", request.getPrice())
                .putToPost("symbol", request.getSymbol()).putToPost("type", request.getType().getCode())
                .putToPost("client-order-id", request.getClientOrderId())
                .putToPost("source", request.getOrderSource().getCode()).putToPost("stop-price", request.getStopPrice())
    ...
    public Long cancelOrder(Long orderId) {
        ...
        InputChecker.checker().shouldNotNull(orderId, "order-id");
    ...
    public Integer cancelOrder(String clientOrderId) {
        ...
        InputChecker.checker().shouldNotNull(clientOrderId, "client-order-id");
    ...
    public BatchCancelOpenOrdersResult batchCancelOpenOrders(BatchCancelOpenOrdersRequest request) {
        ...
        InputChecker.checker().shouldNotNull(request.getAccountId(), "account-id");
        if (request.getSize() != null) {
            InputChecker.checker().checkRange(request.getSize(), 1, 100, "size");
        }
    ...
    public BatchCancelOrderResult batchCancelOrder(List<Long> ids) {
        ...
        InputChecker.checker().shouldNotNull(ids, "order-ids").checkList(ids, 1, 50, "orderIds");

        List<String> stringList = new LinkedList<>();
        for (Object obj : ids) {
            stringList.add(obj.toString());
        }
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("order-ids", stringList);
    ...
    public List<Order> getOpenOrders(OpenOrdersRequest request) {
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("account-id", request.getAccountId())
    ...
    public List<Order> getOrders(OrdersRequest request) {
        ...
        InputChecker.checker().shouldNotNull(request.getSymbol(), "symbol").checkList(request.getStates(), 1, 100,
                "states");
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", request.getSymbol())
                ...
                .putToUrl("end-date", request.getEndDate(), "yyyy-MM-dd").putToUrl("from", request.getStartId())
                .putToUrl("states", request.getStatesString()).putToUrl("size", request.getSize())
                .putToUrl("direct", request.getDirect() == null ? null : request.getDirect().getCode());
    ...
    public List<Order> getOrdersHistory(OrderHistoryRequest request) {
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", request.getSymbol())
                .putToUrl("start-time", request.getStartTime()).putToUrl("end-time", request.getEndTime())
                .putToUrl("size", request.getSize())
    ...
    public List<MatchResult> getMatchResults(MatchResultRequest request) {
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", request.getSymbol())
                .putToUrl("types", request.getTypeString()).putToUrl("start-date", request.getStartDate(), "yyyy-MM-dd")
                .putToUrl("end-date", request.getEndDate(), "yyyy-MM-dd").putToUrl("from", request.getFrom())
                .putToUrl("direct", request.getDirection() == null ? null : request.getDirection().getCode())
                .putToUrl("size", request.getSize());
    ...
    public List<FeeRate> getFeeRate(FeeRateRequest request) {
        ...
        InputChecker.checker().shouldNotNull(request.getSymbols(), "symbol");
    ...
    public void subOrderUpdateV2(SubOrderUpdateV2Request request, ResponseCallback<OrderUpdateV2Event> callback) {
        ...
        HuobiWebSocketConnection.createAssetV2Connection(options, commandList, new OrderUpdateEventV2Parser(), callback,
                false);
    }
    ...
    public void subTradeClearing(SubTradeClearingRequest request, ResponseCallback<TradeClearingEvent> callback) {
        ...
        symbolList.forEach(symbol -> {
            ...
            String topic = WEBSOCKET_TRADE_CLEARING_TOPIC.replace("${symbol}", symbol);
        ...
        HuobiWebSocketConnection.createAssetV2Connection(options, commandList, new TradeClearingEventParser(), callback,
                false);
