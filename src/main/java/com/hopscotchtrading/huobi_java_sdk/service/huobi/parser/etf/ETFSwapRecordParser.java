package com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.etf;
...
import com.hopscotchtrading.huobi_java_sdk.constant.enums.EtfSwapTypeEnum;
...
public class ETFSwapRecordParser implements HuobiModelParser<ETFSwapRecord> {
    ...
    public ETFSwapRecord parse(JSONObject json) {
        return ETFSwapRecord.builder().id(json.getLong("id")).gmtCreated(json.getLong("gmt_created"))
                .currency(json.getString("currency")).amount(json.getBigDecimal("amount"))
                .type(EtfSwapTypeEnum.find(json.getString("type"))).status(json.getInteger("status"))
                .detail(new ETFDetailParser().parse(json.getJSONObject("detail"))).build();
    }
