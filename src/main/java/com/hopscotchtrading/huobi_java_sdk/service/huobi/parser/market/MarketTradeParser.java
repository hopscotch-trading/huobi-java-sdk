...
public class MarketTradeParser implements HuobiModelParser<MarketTrade> {
    ...
    public MarketTrade parse(JSONObject json) {
        ...
        return MarketTrade.builder().id(json.get("id") == null ? null : json.get("id").toString()).tradeId(tradeId)
                .price(new BigDecimal(json.getString("price"))).amount(new BigDecimal(json.getString("amount")))
                .direction(json.getString("direction")).ts(json.getLong("ts")).build();
    }
