...
import com.hopscotchtrading.huobi_java_sdk.model.trade.CancelFailedResult;
...
public class CancelFailedResultParser implements HuobiModelParser<CancelFailedResult> {
    ...
    public CancelFailedResult parse(JSONObject json) {
        return CancelFailedResult.builder().errCode(json.getString("err-code")).errMsg(json.getString("err-msg"))
                .orderId(json.getLong("order-id")).orderState(json.getInteger("order-state")).build();
    }
