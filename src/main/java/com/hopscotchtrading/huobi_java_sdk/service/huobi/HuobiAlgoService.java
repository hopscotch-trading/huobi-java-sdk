...
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.algo.AlgoOrderParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.algo.CancelAlgoOrderResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.algo.CreateAlgoOrderResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.algo.GetHistoryAlgoOrdersResultParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.algo.GetOpenAlgoOrdersResultParser;
...
public class HuobiAlgoService implements AlgoClient {
    ...
    public CreateAlgoOrderResult createAlgoOrder(CreateAlgoOrderRequest request) {
        ...
        InputChecker checker = InputChecker.checker().shouldNotNull(request.getAccountId(), "accountId")
                .shouldNotNull(request.getSymbol(), "symbol").shouldNotNull(request.getOrderSide(), "orderSide")
        ...
        if (request.getOrderType() == AlgoOrderTypeEnum.LIMIT) {
            checker.shouldNotNull(request.getOrderPrice(), "orderPrice").shouldNotNull(request.getOrderSize(),
                    "orderSize");
        } else {
            checker.shouldNotNull(request.getOrderValue(), "orderValue");
        }

        UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("accountId", request.getAccountId())
                .putToPost("symbol", request.getSymbol()).putToPost("orderPrice", request.getOrderPrice())
                .putToPost("orderSide", request.getOrderSide().getSide()).putToPost("orderSize", request.getOrderSize())
                .putToPost("orderValue", request.getOrderValue())
                .putToPost("timeInForce",
                        request.getTimeInForce() == null ? null : request.getTimeInForce().getTimeInForce())
                .putToPost("orderType", request.getOrderType().getType())
                .putToPost("clientOrderId", request.getClientOrderId()).putToPost("stopPrice", request.getStopPrice());
    ...
    public CancelAlgoOrderResult cancelAlgoOrder(CancelAlgoOrderRequest request) {
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("clientOrderIds", request.getClientOrderIds());
    ...
    public GetHistoryAlgoOrdersResult getHistoryAlgoOrders(GetHistoryAlgoOrdersRequest request) {
        ...
        InputChecker.checker().shouldNotNull(request.getSymbol(), "symbol").shouldNotNull(request.getOrderStatus(),
                "orderStatus");
    ...
    public AlgoOrder getAlgoOrdersSpecific(String clientOrderId) {
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("clientOrderId", clientOrderId);
