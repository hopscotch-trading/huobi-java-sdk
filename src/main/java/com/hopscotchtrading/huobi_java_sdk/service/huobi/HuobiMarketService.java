...
import com.hopscotchtrading.huobi_java_sdk.constant.WebSocketConstants;
...
import com.hopscotchtrading.huobi_java_sdk.service.huobi.connection.HuobiWebSocketConnection;
...
import com.hopscotchtrading.huobi_java_sdk.utils.SymbolUtils;
...
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.CandlestickEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.CandlestickParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.CandlestickReqParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketBBOEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDepthEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDepthParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDepthReqParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDetailEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDetailMergedParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDetailParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketDetailReqParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketTickerParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketTradeEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketTradeParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MarketTradeReqParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MbpIncrementalUpdateEventParser;
import com.hopscotchtrading.huobi_java_sdk.service.huobi.parser.market.MbpRefreshUpdateEventParser;
...
public class HuobiMarketService implements MarketClient {
    ...
    public List<Candlestick> getCandlestick(CandlestickRequest request) {
        ...
        // 参数检查
        InputChecker.checker().checkSymbol(request.getSymbol()).checkRange(request.getSize(), 1, 2000, "size")
                .shouldNotNull(request.getInterval(), "CandlestickInterval");

        // 参数构建
        UrlParamsBuilder paramBuilder = UrlParamsBuilder.build().putToUrl("symbol", request.getSymbol())
                .putToUrl("period", request.getInterval().getCode()).putToUrl("size", request.getSize());
    ...
    public void subCandlestick(SubCandlestickRequest request, ResponseCallback<CandlestickEvent> callback) {
        ...
        // 检查参数
        InputChecker.checker().shouldNotNull(request.getSymbol(), "symbol").shouldNotNull(request.getInterval(),
                "interval");
        // 格式化symbol为数组
        ...
        symbolList.forEach(symbol -> {
            ...
            String topic = WEBSOCKET_CANDLESTICK_TOPIC.replace("$symbol$", symbol).replace("$period$",
                    request.getInterval().getCode());
        ...
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new CandlestickEventParser(), callback,
                false);
    }
    ...
    public void subMarketDetail(SubMarketDetailRequest request, ResponseCallback<MarketDetailEvent> callback) {
        ...
        symbolList.forEach(symbol -> {
            ...
            String topic = WEBSOCKET_MARKET_DETAIL_TOPIC.replace("$symbol", symbol);
        ...
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketDetailEventParser(), callback,
                false);
    }
    ...
    public MarketDepth getMarketDepth(MarketDepthRequest request) {
        ...
        // 参数检查
        InputChecker.checker().checkSymbol(request.getSymbol()).shouldNotNull(request.getStep(), "step");

        int size = request.getDepth() == null ? DepthSizeEnum.SIZE_20.getSize() : request.getDepth().getSize();

        // 参数构建
        UrlParamsBuilder paramBuilder = UrlParamsBuilder.build().putToUrl("symbol", request.getSymbol())
                .putToUrl("depth", size).putToUrl("type", request.getStep().getStep());
    ...
    public void subMarketBBO(SubMarketBBORequest request, ResponseCallback<MarketBBOEvent> callback) {
        ...
        symbolList.forEach(symbol -> {
            ...
            String topic = WEBSOCKET_MARKET_BBO_TOPIC.replace("$symbol", symbol);
        ...
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketBBOEventParser(), callback,
                false);

    }

    public void subMbpRefreshUpdate(SubMbpRefreshUpdateRequest request,
            ResponseCallback<MbpRefreshUpdateEvent> callback) {

        // 检查参数
        InputChecker.checker().shouldNotNull(request.getSymbols(), "symbols");
        ...
        symbolList.forEach(symbol -> {
            ...
            String topic = WEBSOCKET_MARKET_MBP_REFRESH_TOPIC.replace("$symbol", symbol).replace("$levels", level + "");
        ...
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MbpRefreshUpdateEventParser(),
                callback, false);
    }
    ...
    public WebSocketConnection subMbpIncrementalUpdate(SubMbpIncrementalUpdateRequest request,
        ...
        String topic = WEBSOCKET_MARKET_MBP_INCREMENT_TOPIC.replace("$symbol", request.getSymbol()).replace("$levels",
                level + "");
        ...
        return HuobiWebSocketConnection.createMarketConnection(options, commandList,
    ...
    public WebSocketConnection reqMbpIncrementalUpdate(SubMbpIncrementalUpdateRequest request,
        ...
        if (level != DepthLevels.LEVEL_150.getLevel()) {
            throw new SDKException(SDKException.INPUT_ERROR,
                    " Unsupport Levels : " + request.getLevels() + " incremental update only support level_150");
        }
        List<String> commandList = new ArrayList<>(1);
    ...
    public void reqCandlestick(ReqCandlestickRequest request, ResponseCallback<CandlestickReq> callback) {
        ...
        String topic = WEBSOCKET_CANDLESTICK_TOPIC.replace("$symbol$", request.getSymbol()).replace("$period$",
                request.getInterval().getCode());
        ...
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new CandlestickReqParser(), callback,
                true);
    }

    public void reqMarketDepth(ReqMarketDepthRequest request, ResponseCallback<MarketDepthReq> callback) {
        // 检查参数
        InputChecker.checker().shouldNotNull(request.getSymbol(), "symbol").shouldNotNull(request.getStep(), "step");

        String topic = WEBSOCKET_MARKET_DEPTH_TOPIC.replace("$symbol", request.getSymbol()).replace("$type",
                request.getStep().getStep());
        ...
        List<String> commandList = new ArrayList<>(1);
        commandList.add(command.toJSONString());
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketDepthReqParser(), callback,
                true);
    ...
    public void reqMarketTrade(ReqMarketTradeRequest request, ResponseCallback<MarketTradeReq> callback) {
        ...
        String topic = WEBSOCKET_MARKET_TRADE_TOPIC.replace("$symbol", request.getSymbol());
        ...
        List<String> commandList = new ArrayList<>(1);
        commandList.add(command.toJSONString());
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketTradeReqParser(), callback,
                true);
    }
    ...
    public void reqMarketDetail(ReqMarketDetailRequest request, ResponseCallback<MarketDetailReq> callback) {
        ...
        String topic = WEBSOCKET_MARKET_DETAIL_TOPIC.replace("$symbol", request.getSymbol());
        ...
        List<String> commandList = new ArrayList<>(1);
        commandList.add(command.toJSONString());
        HuobiWebSocketConnection.createMarketConnection(options, commandList, new MarketDetailReqParser(), callback,
                true);
    }
