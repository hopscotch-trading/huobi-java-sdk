package com.hopscotchtrading.huobi_java_sdk.test;
...
public class PerformanceTest {
    ...
    public static void testCase() {
        ...
        Long startNano = null;
        Long endNano = null;
        ConnectionFactory.NetworkLatency networkLatency = null;
        ...
        // /market/depth
        startNano = System.nanoTime();
        marketClient.getMarketDepth(MarketDepthRequest.builder().symbol(symbol).step(DepthStepEnum.STEP0)
                .depth(DepthSizeEnum.SIZE_5).build());
        endNano = System.nanoTime();
        networkLatency = ConnectionFactory.getLatencyDebugQueue().poll();
        print(networkLatency, startNano, endNano);
        ...
        // /market/kline
        startNano = System.nanoTime();
        marketClient.getCandlestick(
                CandlestickRequest.builder().symbol(symbol).interval(CandlestickIntervalEnum.MIN15).size(5).build());
        endNano = System.nanoTime();
        networkLatency = ConnectionFactory.getLatencyDebugQueue().poll();
        print(networkLatency, startNano, endNano);
        ...
        // 下单
        startNano = System.nanoTime();
        Long orderId = tradeClient.createOrder(
                CreateOrderRequest.spotBuyLimit(accountId, symbol, new BigDecimal("3"), new BigDecimal("2")));
        endNano = System.nanoTime();
        networkLatency = ConnectionFactory.getLatencyDebugQueue().poll();
        print(networkLatency, startNano, endNano);
    ...
    public static void print(ConnectionFactory.NetworkLatency networkLatency, Long startNanoTime, Long endNanoTime) {
