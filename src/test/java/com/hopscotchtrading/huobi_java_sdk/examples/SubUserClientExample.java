...
public class SubUserClientExample {
    ...
    public static void main(String[] args) {
        ...
        SubUserClient subUserClient = SubUserClient
        ...
        long outTransferId = subUserClient
                .transferSubuser(TransferSubuserRequest.builder().subUid(subUid).currency("usdt")
                        .amount(new BigDecimal("10")).type(TransferMasterTypeEnum.MASTER_TRANSFER_OUT).build());
        ...
        long inTransferId = subUserClient.transferSubuser(TransferSubuserRequest.builder().subUid(subUid)
                .currency("usdt").amount(new BigDecimal("10")).type(TransferMasterTypeEnum.MASTER_TRANSFER_IN).build());
        ...
        creationParamList.add(SubUserCreationParam.builder().userName("test" + System.nanoTime() + "1").build());
        creationParamList.add(SubUserCreationParam.builder().userName("test" + System.nanoTime() + "2").build());

        List<SubUserCreationInfo> creationInfoList = subUserClient
                .subuserCreation(SubUserCreationRequest.builder().userList(creationParamList).build());

        creationInfoList.forEach(info -> {
            System.out.println(info.toString());
        });

        GetSubUserListResult getSubUserListResult = subUserClient
                .getSubUserList(GetSubUserListRequest.builder().build());
        ...
        SubUserManagementResult subUserManagementResult = subUserClient.subuserManagement(
                SubUserManagementRequest.builder().subUid(subUid).action(SubUserManagementActionEnum.UNLOCK).build());

        System.out.println(subUserManagementResult);

        GetSubUserAccountListResult getSubUserAccountListResult = subUserClient
                .getSubuserAccountList(GetSubUserAccountListRequest.builder().subUid(subUid).build());
        ...
        SubUserTransferabilityResult transferabilityResult = subUserClient
                .subuserTransferability(transferabilityRequest);
        if (transferabilityResult != null) {
            for (SubUserTransferabilityState transferabilityState : transferabilityResult.getList()) {
                System.out.println("transferabilityState:  " + "subUid:" + transferabilityState.getSubUid()
                        + "  accountType:" + transferabilityState.getAccountType() + "   transferrable: "
                        + transferabilityState.getTransferrable());
            }
        }
        ...
        if (tradableMarketResult != null) {
            for (SubUserTradableMarketState marketState : tradableMarketResult.getList()) {
                System.out.println("tradable market State:  " + "subUid:" + marketState.getSubUid() + "  accountType:"
                        + marketState.getAccountType() + "   activation: " + marketState.getActivation());
            }
        }

        String googleAuthKey = "";
        String optToken = GoogleAuthHelper.getVercodeTime(googleAuthKey) + "";
        String permission = SubUserApiKeyPermissionEnums.READ_ONLY.getPermission() + ","
                + SubUserApiKeyPermissionEnums.TRADE.getPermission();
        ...
        SubUserApiKeyGenerationResult apiKeyGenerationResult = subUserClient
                .subuserApiKeyGeneration(apiKeyGenerationRequest);
        if (apiKeyGenerationResult != null) {
            System.out.println("api key generation ::" + JSON.toJSONString(apiKeyGenerationResult));
        }

        GetApiKeyListResult getApiKeyListResult = subUserClient
                .getApiKeyList(GetApiKeyListRequest.builder().uid(subUid).build());
        ...
        SubUserApiKeyModificationResult apiKeyModificationResult = subUserClient
                .subuserApiKeyModification(apiKeyModificationRequest);
        if (apiKeyModificationResult != null) {
            System.out.println("api key modification ::" + JSON.toJSONString(apiKeyModificationResult));
        }

        getApiKeyListResult = subUserClient
                .getApiKeyList(GetApiKeyListRequest.builder().uid(subUid).accessKey(newApiKey).build());
        getApiKeyListResult.getList().forEach(info -> {
            System.out.println(info);
        });

        subUserClient.subuserApiKeyDeletion(
                SubUserApiKeyDeletionRequest.builder().subUid(subUid).accessKey(newApiKey).build());
        ...
        GetSubUserDepositResult getSubUserDepositResult = subUserClient
                .getSubUserDeposit(GetSubUserDepositRequest.builder().subUid(subUid).build());
