...
import com.hopscotchtrading.huobi_java_sdk.client.TradeClient;
...
public class TradeClientExample {
    ...
    public static void main(String[] args) {
        ...
        BigDecimal bidPrice = marketDetailMerged.getBid().getPrice().multiply(new BigDecimal("0.95")).setScale(4,
                RoundingMode.DOWN);
        BigDecimal askPrice = marketDetailMerged.getAsk().getPrice().multiply(new BigDecimal("1.05")).setScale(4,
                RoundingMode.DOWN);

        CreateOrderRequest buyLimitRequest = CreateOrderRequest.spotBuyLimit(spotAccountId, clientOrderId, symbol,
                bidPrice, new BigDecimal("2"));
        Long buyLimitId = tradeService.createOrder(buyLimitRequest);
        System.out.println("create buy-limit order:" + buyLimitId);

        CreateOrderRequest sellLimitRequest = CreateOrderRequest.spotSellLimit(spotAccountId, symbol, askPrice,
                new BigDecimal("2"));
        Long sellLimitId = tradeService.createOrder(sellLimitRequest);
        System.out.println("create sell-limit order:" + sellLimitId);

        CreateOrderRequest buyMarketRequest = CreateOrderRequest.spotBuyMarket(spotAccountId, symbol,
                new BigDecimal("5"));
        Long buyMarketId = tradeService.createOrder(buyMarketRequest);
        System.out.println("create buy-market order:" + buyMarketId);

        CreateOrderRequest sellMarketRequest = CreateOrderRequest.spotSellMarket(spotAccountId, symbol,
                new BigDecimal("1"));
        ...
        CreateOrderRequest superMarginBuyLimitRequest = CreateOrderRequest.superMarginBuyLimit(superMarginAccountId,
                superMarginSymbol, new BigDecimal("0.17"), new BigDecimal("30"));
        ...
        CreateOrderRequest superMarginBuyMarketRequest = CreateOrderRequest.superMarginBuyMarket(superMarginAccountId,
                superMarginSymbol, new BigDecimal("5"));
        ...
        CreateOrderRequest superMarginSellLimitRequest = CreateOrderRequest.superMarginSellLimit(superMarginAccountId,
                superMarginSymbol, new BigDecimal("0.2"), new BigDecimal("30"));
        ...
        CreateOrderRequest superMarginSellMarketRequest = CreateOrderRequest.superMarginSellMarket(superMarginAccountId,
                superMarginSymbol, new BigDecimal("20"));
        ...
        CreateOrderRequest marginBuyLimitRequest = CreateOrderRequest.marginBuyLimit(marginXrpAccountId, marginSymbol,
                new BigDecimal("0.17"), new BigDecimal("30"));
        ...
        CreateOrderRequest marginBuyMarketRequest = CreateOrderRequest.marginBuyMarket(marginXrpAccountId, marginSymbol,
                new BigDecimal("5.5"));
        ...
        CreateOrderRequest marginSellLimitRequest = CreateOrderRequest.marginSellLimit(marginXrpAccountId, marginSymbol,
                new BigDecimal("0.295"), new BigDecimal("20"));
        ...
        CreateOrderRequest marginSellMarketRequest = CreateOrderRequest.marginSellMarket(marginXrpAccountId,
                marginSymbol, new BigDecimal("30"));
        ...
        CreateOrderRequest buyStopLossRequest = CreateOrderRequest.buyStopLoss(spotAccountId, symbol,
                bidPrice.multiply(new BigDecimal("1.5")).setScale(4, RoundingMode.DOWN), new BigDecimal("1"),
                bidPrice.multiply(new BigDecimal("1.5")).setScale(4, RoundingMode.DOWN), StopOrderOperatorEnum.GTE,
                OrderSourceEnum.API);
        ...
        CreateOrderRequest sellStopLossRequest = CreateOrderRequest.sellStopLoss(spotAccountId, symbol,
                askPrice.multiply(new BigDecimal("0.95")).setScale(4, RoundingMode.DOWN), new BigDecimal("1"),
                askPrice.multiply(new BigDecimal("0.95")).setScale(4, RoundingMode.DOWN), StopOrderOperatorEnum.LTE,
                OrderSourceEnum.API);
        ...
        List<Order> orderList = tradeService
                .getOpenOrders(OpenOrdersRequest.builder().accountId(spotAccountId).symbol(symbol).build());
        ...
        BatchCancelOpenOrdersResult result = tradeService.batchCancelOpenOrders(
                BatchCancelOpenOrdersRequest.builder().accountId(spotAccountId).symbol(symbol).build());
        System.out.println("spot batchCancel open Orders:" + result.toString());

        BatchCancelOpenOrdersResult result1 = tradeService.batchCancelOpenOrders(
                BatchCancelOpenOrdersRequest.builder().accountId(marginXrpAccountId).symbol(marginSymbol).build());
        System.out.println("margin xrp batchCancel open Orders:" + result1.toString());

        BatchCancelOpenOrdersResult result2 = tradeService.batchCancelOpenOrders(
                BatchCancelOpenOrdersRequest.builder().accountId(superMarginAccountId).symbol(marginSymbol).build());
        ...
        List<Order> ordersList = tradeService
                .getOrders(OrdersRequest.builder().symbol(symbol).states(stateList).build());

        ordersList.forEach(order -> {
            System.out.println(new Date(order.getCreatedAt()) + ":" + order.toString());
        });

        List<Order> historyOrderList = tradeService.getOrdersHistory(OrderHistoryRequest.builder().symbol(symbol)
                .startTime(1565107200000L).direction(QueryDirectionEnum.PREV).build());
        ...
        List<MatchResult> matchResultList1 = tradeService
                .getMatchResults(MatchResultRequest.builder().symbol(symbol).build());
