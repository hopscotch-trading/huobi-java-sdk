...
import com.hopscotchtrading.huobi_java_sdk.model.market.PriceLevel;
...
public class OrderBookExamples {
    ...
    private void incrementUpdateTask(MbpIncrementalUpdateEvent event) {
        ...
        if (WebSocketConstants.ACTION_REP.equals(event.getAction())) {
            ...
            for (MbpIncrementalUpdateEvent eve : preUpdateList) {
                ...
                System.out.println("event:::: preSeqNum:" + preSeqNum + "  seqNum:" + eve.getSeqNum() + "  snapshot:"
                        + snapshotSeqNum);
                ...
                // pre < seq 则忽略
                if (preSeqNum.compareTo(snapshotSeqNum) < 0) {
                    System.out.println(" ignore message: preSeqNum:" + preSeqNum + "  seqNum:" + eve.getSeqNum()
                ...
                if (preSeqNum.compareTo(snapshotSeqNum) > 0) {
                    System.out.println("find incr message preSeqNum > snapshot seqNum....    message:" + preSeqNum
                            + "   snapshot:" + snapshotSeqNum);
                    break;
                }
    ...
    private void sendReqMessage() {
        marketClient.reqMbpIncrementalUpdate(SubMbpIncrementalUpdateRequest.builder().symbol(symbol).build(),
                connection);
    }
    ...
    public MbpIncrementalUpdateEvent getDepth() {
        ...
        return MbpIncrementalUpdateEvent.builder().asks(askLevelList).bids(bidLevelList).build();
    }
    ...
    public static void main(String[] args) {
        ...
        scheduledExecutorService.scheduleAtFixedRate(() -> {
            try {
                ...
                System.out.println("----------------------------");
                System.out.println("ask1:  " + askLevel.getPrice().toPlainString() + " ------ "
                        + askLevel.getAmount().toPlainString());
                System.out.println("bid1:  " + bidLevel.getPrice().toPlainString() + " ------ "
                        + bidLevel.getAmount().toPlainString());
                System.out.println("----------------------------");
            } catch (Exception e) {
                e.printStackTrace();
            }
